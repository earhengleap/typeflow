import React from "react";

interface AuthenticInfoProps extends React.SVGProps<SVGSVGElement> {
    size?: number;
}

export const AuthenticInfo: React.FC<AuthenticInfoProps> = ({ size, className, ...props }) => {
    return (
        <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 640 640"
            width={size || 24}
            height={size || 24}
            className={className}
            {...props}
        >
            {/* !Font Awesome Free v5.15.4/6.x by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2026 Fonticons, Inc. */}
            <path
                fill="currentColor"
                d="M272 112C272 85.5 293.5 64 320 64C346.5 64 368 85.5 368 112C368 138.5 346.5 160 320 160C293.5 160 272 138.5 272 112zM224 256C224 238.3 238.3 224 256 224L320 224C337.7 224 352 238.3 352 256L352 512L384 512C401.7 512 416 526.3 416 544C416 561.7 401.7 576 384 576L256 576C238.3 576 224 561.7 224 544C224 526.3 238.3 512 256 512L288 512L288 288L256 288C238.3 288 224 273.7 224 256z"
            />
        </svg>
    );
};
